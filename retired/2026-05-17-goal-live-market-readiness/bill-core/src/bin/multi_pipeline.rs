            ...
            for i in 30..bars.len() {
                ...
                if spike_vol && near_high {
                    ...
                    if rr >= 1.5 {
                        ...
                        trades.push(Trade {
                            strategy_id: "wq-alpha-009".into(), symbol: bars[i].symbol.clone(),
                            side: "short".into(), entry: bars[i].close, exit: exit_close,
                            entry_ts: bars[i].ts.clone(), exit_ts, r_multiple: r, contracts: 1,
                        });
                    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (symbol, bars) in &by_symbol {
        ...
        for sid in &strategies {
            ...
            if total > 0 {
                println!("  {}: {} trades, {}/{} W/L ({:.1}%), total R {:.2}",
                    sid, total, wins, losses, wins as f64 / total as f64 * 100.0, total_r);
                sym_total_r += total_r;
        ...
        let all_trades: Vec<Trade> = by_symbol.get(symbol).map(|_| {
        ...
        if !all_trades.is_empty() {
            ...
            let avg_risk = all_trades.iter().map(|t| (t.entry - t.exit).abs()).sum::<f64>() / total as f64;
            let nq_point_value = if symbol == "NQ" { 20.0 } else if symbol == "ES" { 50.0 } else { 10.0 };
            let gross_pnl = pnl_r * avg_risk * nq_point_value;
            let friction = total as f64 * 5.0; // $5 round trip
            let net_pnl = gross_pnl - friction;

            println!("  Combined (filtered): {} trades, {}/{} W/L ({:.1}% WR), PnL +${:.0}",
    ...
    println!("=== TOTAL ===");
    println!("All trades: {}, Net PnL: +${:.0}", grand_total_trades, grand_total_pnl);
