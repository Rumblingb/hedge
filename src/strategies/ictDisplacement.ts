...
function buildSignal(args: {
  ...
  const { context, side, stop, target, confidence, barIntervalMinutes, liquidityPoolLevel, fairValueGapBoundary } = args;
  ...
  return {
    ...
    side, entry, stop, target, rr, confidence,
    contracts: 1,
    maxHoldMinutes: 20,
    meta: {
...
export class IctDisplacementStrategy implements Strategy {
  ...
  public readonly description = "ICT-style liquidity sweep, displacement, and fair value gap continuation. Confidence thresholds relaxed for higher signal density.";
  ...
  public generateSignal(context: StrategyContext): StrategySignal | null {
    ...
    if (!dailyLike && category !== "index" && category !== "fx") return null;
    ...
    if (!dailyLike) {
      ...
      if (sessionMinute < 5 || sessionMinute > maxSessionMinute) return null;
    }
    ...
    if (liquidityHistory.length < Math.max(4, lookback - 2)) return null;
    ...
    const displacementThreshold = Math.max(0.0001, atr * 0.40, avgBody * 1.0);
    if (atr > 0 && barRange > (atr * context.config.tuning.volatilityKillAtrMultiple)) return null;

    const hmmRegime = context.macro?.hmmRegime;
    let regimeMultiplier = 1.0;
    if (hmmRegime && hmmRegime !== "trending") regimeMultiplier = 0.6;
    ...
    const fvgTolerance = atr * 0.5;
    const bullishFairValueGap = context.bar.low >= first.high - fvgTolerance;
    const bearishFairValueGap = context.bar.high <= first.low + fvgTolerance;
    ...
    if (bullishSweep && bullishDisplacement && bullishFairValueGap && bullishStructureShift) {
      ...
      let confidence = 0.55 * regimeMultiplier;  // was 0.81
      if (confidence < 0.20) return null;        // was 0.35
