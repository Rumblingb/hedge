...
function unwrapReview(review: PredictionCycleReview | { review?: PredictionCycleReview | null } | null): PredictionCycleReview | null {
  if (review && "review" in review) {
    return review.review ?? null;
  }
  return review as PredictionCycleReview | null;
}

function candidateCounts(review: PredictionCycleReview | { review?: PredictionCycleReview | null } | null): Record<string, number> {
  const unwrapped = unwrapReview(review);
  return {
    reject: unwrapped?.counts?.reject ?? 0,
    watch: unwrapped?.counts?.watch ?? 0,
    "paper-trade": unwrapped?.counts?.["paper-trade"] ?? 0
  };
}
...
export async function buildEdgeForensics(args: {
  ...
  const [markets, review, macroPolicy, strategyFactory] = await Promise.all([
    ...
    readJsonSafe<PredictionCycleReview | { review?: PredictionCycleReview | null }>(predictionReviewPath),
  ...
  const reviewCounts = candidateCounts(review);
  const paperCandidates = reviewCounts["paper-trade"] ?? 0;
  ...
  const predictionRootCauses = [
    ...
    ...((reviewCounts.watch ?? 0) === 0 ? ["no watch candidates survived contract matching"] : []),
