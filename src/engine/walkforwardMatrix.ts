...
async function evaluateConfig(args: {
  ...
  newsGate: NewsGate;
  profiles: typeof RESEARCH_PROFILES;
  mode: WalkforwardMatrixMode;
  trainDays: number;
  testDays: number;
  embargoDays: number;
...
export async function buildWalkforwardMatrixReport(args: {
  ...
  const configuredMaxProfiles = args.maxProfiles ?? Number.parseInt(process.env.BILL_WALKFORWARD_MATRIX_MAX_PROFILES ?? String(RESEARCH_PROFILES.length), 10);
  const profileLimit = Math.max(1, Math.min(RESEARCH_PROFILES.length, Number.isFinite(configuredMaxProfiles) ? configuredMaxProfiles : RESEARCH_PROFILES.length));
  const profiles = RESEARCH_PROFILES.slice(0, profileLimit);
  const configsToRun = [
