...
function topCandidate(rows: PredictionCandidate[], recentCycles: Array<Record<string, unknown>> = []): PredictionCycleReview["topCandidate"] {
  ...
  return {
    ...
    liquidityImpactPct: first.sizing?.liquidity?.impactPenaltyPct,
    postImpactEdgePct: first.sizing?.liquidity?.postImpactEdgePct,
    fillQuality: first.sizing?.liquidity?.fillQuality,
