...
async function enrichWithClobBooks(
  ...
  await Promise.all(tokenRows.map(async ({ snapshot, index }) => {
    try {
      ...
      // Keep Gamma-only snapshots usable when the public CLOB endpoint is unavailable.
    }
  }));
...
export async function fetchPolymarketLiveSnapshot(limit = 10): Promise<PredictionMarketSnapshot[]> {
  ...
  return enrichWithClobBooks(
    snapshots,
    Number.parseInt(process.env.BILL_POLYMARKET_CLOB_ENRICH_LIMIT ?? String(Math.max(limit, 25)), 10)
  );
}
