...
export function buildBillSourceCatalog(env: NodeJS.ProcessEnv, policy: BillTrackPolicy): BillSourceStatus[] {
  ...
  const definitions: BillSourceDefinition[] = [
    ...
    {
      id: "polymarket-public-trader-data",
      name: "Polymarket Public Trader Data",
      category: "prediction-market",
      tracks: ["prediction"],
      collectionKinds: ["prediction", "realtime", "historical", "discovery"],
      access: "public",
      priority: "supplemental",
      automationReady: true,
      reference: "Bill native collector + public Polymarket leaderboard/activity/positions endpoints",
      collectionCommand: "npm run bill:prediction-copy-demo",
      trainingUse: "Tracks outsized-return wallets, live consensus positioning, and copy-demo ideas while the core cross-venue edge lane stays selective.",
      reason: () => "Public trader surfaces are now wired into Bill as a bounded copy-demo lane for prediction-market revenue seeking."
    },
    {
    ...
    {
      id: "prediction-market-analysis-dataset",
      name: "Prediction Market Analysis Dataset",
      category: "prediction-market",
      tracks: ["prediction"],
      collectionKinds: ["prediction", "historical"],
      access: "local-package",
      priority: "supplemental",
      automationReady: false,
      reference: "GitHub jon-becker/prediction-market-analysis",
      collectionCommand: "npm run bill:prediction-market-analysis-status",
      trainingUse: "Large historical Kalshi and Polymarket market/trade parquet corpus for calibration, maker/taker edge, price-bucket win-rate, and backtest research.",
      reason: () => "High-value backtest dataset. A bounded local importer is available, but the large external corpus stays isolated until data root, DuckDB, licensing, and disk budget are validated."
    },
    {
      id: "pykalshi-sdk",
      name: "pykalshi SDK",
    ...
    },
    {
      ...
      category: "discovery",
      tracks: ["prediction", "futures-core", "options-us", "crypto-liquid", "macro-rates", "long-only-compounder"],
    ...
    },
    {
      ...
      category: "universe",
      tracks: ["futures-core", "options-us", "crypto-liquid", "macro-rates", "long-only-compounder"],
    ...
    },
    {
      ...
      collectionKinds: ["bars", "historical", "realtime", "futures", "crypto", "macro"],
      access: "public",
      priority: "primary",
      automationReady: true,
      reference: "Bill freeSources yahoo adapter",
      collectionCommand: "npm run bill:research-collect",
      trainingUse: "Cheap futures, liquid-crypto, VIX/rates/credit proxy context bars for regime labeling, market-state features, and daily retraining inputs.",
      reason: () => "Bill already uses Yahoo as the zero-key default for public futures, liquid-crypto, and free macro proxy bars."
    },
    {
      id: "yahoo-macro-context-free",
      name: "Yahoo Free Macro Context",
      category: "macro",
      tracks: ["macro-rates", "futures-core", "options-us"],
      collectionKinds: ["macro", "historical", "bars"],
      access: "public",
      priority: "primary",
      automationReady: true,
      reference: "Bill macro-context-free command",
      collectionCommand: "npx tsx src/cli.ts macro-context-free",
      trainingUse: "Zero-key VIX, VIX3M, Treasury-yield, dollar, credit, equity, and bond ETF proxies for tail-risk gates.",
      reason: () => "No API key required; use as a context gate and sizing reducer until keyed FRED/options data is configured."
    },
    {
      id: "noaa-swpc-space-weather",
      name: "NOAA SWPC Space Weather",
      category: "macro",
      tracks: ["futures-core", "macro-rates"],
      collectionKinds: ["macro", "realtime"],
      access: "public",
      priority: "supplemental",
      automationReady: false,
      reference: "swpc.noaa.gov public products",
      trainingUse: "Catalog-only abnormal-risk veto candidate from the founder's live-data image; useful only if geomagnetic shock days explain measured strategy failure.",
      reason: () => "Free public source, but not wired until a research contract proves it explains futures edge or drawdown."
    },
    {
      ...
      priority: "supplemental",
      automationReady: false,
      reference: "earthquake.usgs.gov public feeds",
      trainingUse: "Catalog-only geopolitical/commodity shock context for major supply-chain or regional risk events.",
      reason: () => "Free public source, but too indirect for first-lane intraday futures unless a measured failure mode requires it."
    },
    {
      id: "opensky-network-air-traffic",
      name: "OpenSky Network Air Traffic",
      category: "macro",
      tracks: ["futures-core", "macro-rates"],
      collectionKinds: ["macro", "realtime"],
      access: "public",
      priority: "supplemental",
      automationReady: false,
      reference: "opensky-network.org public API",
      trainingUse: "Catalog-only transport-disruption proxy for macro/event regimes.",
      reason: () => "Free source from the image, but should stay out of trading logic until it improves OOS regime classification."
    },
    {
      id: "stooq-daily-bars",
    ...
    },
    {
      ...
      category: "market-data",
      tracks: ["futures-core", "options-us", "long-only-compounder"],
    ...
    },
    {
      ...
      category: "market-data",
      tracks: ["options-us", "crypto-liquid", "long-only-compounder"],
    ...
    },
    {
      ...
      name: "IEX Cloud",
      category: "market-data",
      tracks: ["options-us", "long-only-compounder"],
    ...
    },
    {
      ...
      name: "OpenFIGI",
      category: "symbology",
      tracks: ["futures-core", "options-us", "macro-rates", "long-only-compounder"],
    ...
    },
    {
      ...
      name: "SEC EDGAR",
      category: "filings",
      tracks: ["options-us", "macro-rates", "long-only-compounder"],
