...
export function parseYahooMacroChart(args: {
  payload: unknown;
}): FreeMacroPoint[] {
...
export function deriveFreeMacroContext(series: FreeMacroSeries[]): FreeMacroContextReport["derived"] {
  ...
  const vixTermStructure = vix3mMinusVix === null
    ? "unknown"
    : vix3mMinusVix < 0
      ? "backwardation"
      : "contango";
  ...
  const creditSpreadProxy20d = hyg?.change20dPct != null && lqd?.change20dPct != null
    ...
    : null;
  const creditRiskProxy = creditSpreadProxy20d === null
    ? "unknown"
    : creditSpreadProxy20d < -2
      ? "weakening"
      : "normal";
  const equityTrendProxy = spy?.change20dPct === null
    ? "unknown"
    : (spy?.change20dPct ?? 0) < -4
      ? "risk-off"
      : "risk-on";
  ...
  const riskRegime = roundedTailScore >= 65 ? "stress" : roundedTailScore >= 35 ? "elevated" : "normal";
  if (notes.length === 0) {
    notes.push("No free macro proxy is currently flagging acute tail stress.");
  }
...
async function fetchYahooMacroSeries(args: {
  ...
  try {
    ...
    const response = await fetch(url, {
      ...
      headers: { "user-agent": "rumbling-hedge/0.1" }
    });
    if (!response.ok) {
...
export async function buildFreeMacroContextReport(args: {
  ...
  const series = await Promise.all(DEFAULT_SERIES.map((item) => fetchYahooMacroSeries({
    ...item,
    range,
    timeoutMs
  })));
