#!/usr/bin/env python3
"""Replay a fixed CLOB quote-update-intensity feature.

Research-only. This changes one variable from the rejected CLOB drift baseline:
only trade the signed recent quote drift when quote updates have been unusually
active in a fixed lookback window. The thresholds are fixed up front; the script
does not mine parameters or approve paper/live routing.
"""
...
OUT = STATE / "prediction-clob-quote-intensity-replay.latest.json"
VAULT = Path.home() / "Documents/memorybrain"
OUT_MD = VAULT / "Agent-Hermes" / "prediction-clob-quote-intensity-replay-2026-05-30.md"
    ...
    for asset_rows in quotes.values():
        ...
        for idx, row in enumerate(asset_rows):
            while left < idx and asset_rows[left]["tsMs"] < row["tsMs"] - lookback_ms:
                left += 1
            lookback = asset_rows[left:idx]
            if len(lookback) < min_updates or row["spread"] > max_start_spread:
                continue
            prior_move = row["mid"] - lookback[0]["mid"]
            ...
            samples.append({
                ...
                "updatesInLookback": len(lookback),
...
def build_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "decision": "watch-research-only-not-paper" if watch else "research-only-no-quote-intensity-edge",
        "nextAction": "Join to resolved labels and fee/fillability before paper review." if watch else "Do not rerun this exact quote-intensity fixed form without a new label/source/feature.",
    }


def render_markdown(payload: dict[str, Any]) -> str:
    lines = [
        "# Prediction CLOB Quote Intensity Replay - 2026-05-30",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Replay fixed CLOB quote-update-intensity feature.")
