...
def futures_frontier(catalog: dict[str, Any], futures_no_edge: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    items = [
        ...
        base_item(
            ...
            lane="futures",
            priority=21,
            one_variable="options regime overlay",
            hypothesis="Daily SPX options regime may improve NQ trade selection as a risk overlay, not an entry signal.",
            evidence=[
                "COT/TFF current gate is no-edge; a different non-price regime source is required",
                "external catalog has SPX options daily regime features with explicit guardrails",
            ],
            commands=[
                "Join sp500_options_daily_regime to futures OOS windows as the only new variable.",
                "Reject if improvement is only from blocking all trades or reducing sample below the OOS contract.",
            ],
            promotion_gate="Overlay must improve OOS expectancy/drawdown after costs without reducing sample below the promotion contract.",
