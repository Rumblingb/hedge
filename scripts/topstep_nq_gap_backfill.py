#!/usr/bin/env python3
"""Read-only NQ 1m history backfill from TopstepX /api/History/retrieveBars.

Pulls 1m bars across the contract roll (M26 -> U26), stitches by daily volume
...
Retention note (probed 2026-06-13): TopstepX serves ~2 months of 1m history on
the front contracts; fully expired contracts return nothing at <=15m granularity.
"""
...
OUT_DEFAULT = Path.home() / "hedge/data/free/NQ-1m-gap-2026.csv"
...
def main():
    ...
    token = get_token()
    contracts = ["CON.F.US.ENQ.M26", "CON.F.US.ENQ.U26"]
    ...
    print(f"roll to U26 from: {roll_days[0] if roll_days else 'n/a'}", file=sys.stderr)
    ...
    with out_path.open("w") as f:
        ...
        for ts in sorted(rows):
            ...
            if not t.endswith("Z"):
                t += "Z"
            f.write(f"{t},NQ,{b['o']},{b['h']},{b['l']},{b['c']},{int(b.get('v') or 0)}\n")
