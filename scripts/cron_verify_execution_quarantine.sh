...
RESULT=$(python3 scripts/verify_execution_quarantine.py 2>&1)
if echo "$RESULT" | python3 -c "import json,sys; d=json.load(sys.stdin); exit(0 if d.get('allChecksPassed') else 1)" 2>/dev/null; then
    exit 0
else
    echo "FAILED: verify_execution_quarantine"
    echo "$RESULT"
    exit 1
fi
