#!/usr/bin/env python3
"""Read-only ES 1m history backfill from TopstepX /api/History/retrieveBars.
...
Mirrors scripts/topstep_nq_gap_backfill.py for the ES (E-mini S&P 500) contract.
"""
...
OUT_DEFAULT = Path.home() / "hedge/data/free/ES-1m-gap-2026.csv"
...
def main():
    ...
    token = get_token()
    contracts = ["CON.F.US.EP.M26", "CON.F.US.EP.U26"]
    ...
    with out_path.open("w") as f:
        ...
        for ts in sorted(rows):
            ...
            if not t.endswith("Z"):
                t += "Z"
            f.write(f"{t},ES,{b['o']},{b['h']},{b['l']},{b['c']},{int(b.get('v') or 0)}\n")
