...
fn run_orb_breakout(
    ...
    for (sym, group) in &symbol_groups {
        ...
        for si in 0..group.len() {
            ...
            let range_high = group[session_start..session_start + range_window]
                .iter()
                .map(|(_, b)| b.high)
                .fold(0.0_f64, f64::max);
            let range_low = group[session_start..session_start + range_window]
                .iter()
                .map(|(_, b)| b.low)
            ...
            let avg_vol: f64 = group[si.saturating_sub(10)..si]
                .iter()
                .map(|(_, b)| b.volume as f64)
                .sum::<f64>()
                / 10.0;
            if avg_vol <= 0.0 {
                continue;
            }
            if (bar.volume as f64) < avg_vol * vol_threshold {
                continue;
            }

            if let Some(&(exit_idx, exit_bar)) = group.get(si + exit_offset) {
                let atr = group[si.saturating_sub(14)..si]
                    .iter()
                    .map(|(_, b)| b.high - b.low)
                    .sum::<f64>()
                    / 14.0;
                if atr <= 0.0 {
                    continue;
...
fn run_daily_trend(bars: &[Bar]) -> Vec<Trade> {
    ...
    for (sym, group) in &symbol_groups {
        ...
        for si in 50..group.len() - 1 {
            ...
            if sma20 <= 0.0 || sma50 <= 0.0 {
                continue;
            }

            let prev_close = group[si - 1].1.close;
            let atr: f64 = group[si - 14..si]
                .iter()
                .map(|(_, b)| b.high - b.low)
                .sum::<f64>()
                / 14.0;
...
fn simulate_prop_firm(
    ...
    let all_profits: Vec<f64> = daily_pnl
        .values()
        .flat_map(|v| v.iter())
        .filter(|&&v| v > 0.0)
        .copied()
    ...
    let consistency_ratio = if total_profit > 0.0 {
        best_day_profit / total_profit
    } else {
        0.0
    };
    ...
    for day in days {
        ...
        let day_trades: Vec<f64> = day_trades_all
            .map(|t| {
                t.iter()
                    .take(phase.max_trades_per_day)
                    .copied()
                    .collect::<Vec<f64>>()
            })
            .unwrap_or_default();
        ...
        if sim_equity >= rules.profit_target && days_traded >= rules.min_trading_days {
            let run_profits: Vec<f64> = daily_pnl
                .values()
                .take(days_traded)
                .flat_map(|v| v.iter())
                .filter(|&&v| v > 0.0)
            ...
            let run_consistency = if run_total > 0.0 {
                run_best / run_total
            } else {
                0.0
            };
    ...
    let max_consecutive_losses = trades
        .iter()
        .fold((0usize, 0usize), |(max_curr, curr), t| {
            if t.r_multiple <= 0.0 {
                (max_curr.max(curr + 1), curr + 1)
            } else {
                (max_curr, 0)
            }
        })
    ...
    let avg_loss: f64 = trades
        ...
        .sum::<f64>()
        / (total - wins).max(1) as f64;
    let kelly = if avg_loss > 0.0 {
        win_rate - (1.0 - win_rate) / (avg_win / avg_loss).max(0.01)
    } else {
        0.0
    };

    let net_pnl = sim_equity;
    let gross_pnl: f64 = trades
        .iter()
        .map(|t| (t.entry - t.exit).abs() * point_value * phase.position_size_mult - 5.0)
        .sum();
    ...
    println!(
        "  Kelly: {:.2}% | Max consecutive losses: {}",
        kelly * 100.0,
        max_consecutive_losses
    );
    println!(
        "  Net PnL: ${:.0} (${:.0} gross, {} trades × $5 comm)",
        net_pnl, gross_pnl, total
    );
    println!(
        "  Max trades/day: {} | Size: {}×",
        phase.max_trades_per_day, phase.position_size_mult
    );
    println!(
        "  Target RR: {:.2} | Daily lock: ${:.0}/${:.0}",
        phase.target_rr, phase.daily_profit_lock, phase.daily_loss_lock
    );
    println!(
        "  Days: {}, Best day: ${:.0} ({:.1}%)",
        days_traded,
    ...
    println!(
        "  Consistency: {} {:.1}% ≤ {:.0}% rule",
        if consistency_ratio <= rules.max_best_day_pct {
            "✅"
        } else {
            "❌"
        },
        consistency_ratio * 100.0,
        rules.max_best_day_pct * 100.0
    );

    if dll_breached {
        println!(
            "  ⚠️  DLL: soft breach (${:.0}/day) — account lives",
            rules.daily_loss_limit
        );
    }
    if failed_dd {
        println!(
            "  💀 FAIL: trailing DD exceeded ${:.0} in {} days",
            rules.max_drawdown, days_traded
        );
    } else if passed {
        println!(
            "  🏆 PASS: ${:.0} in {} trading days (target: ${:.0})",
            sim_equity,
            days_traded.min(days.len()),
            rules.profit_target
        );
    } else if sim_equity >= rules.profit_target {
        println!(
            "  ⏳ HIT target but consistency check pending (need {} days)",
            rules.min_trading_days
        );
        println!(
            "  🏆 CONSIDERED PASS: ${:.0} in {} days",
            sim_equity, days_traded
        );
    } else {
        println!(
            "  ⏳ In progress: ${:.0} / ${:.0} profit target ({} days)",
            sim_equity, rules.profit_target, days_traded
        );
    }
}
...
fn main() {
    ...
    if bars.is_empty() {
        eprintln!("No bars loaded");
        return;
    }

    let point_value = if bars.iter().any(|b| b.symbol == "ES") {
        50.0
    } else {
        20.0
    };
    ...
    println!(
        "Target: {} (${:.0}/pt), CSV: {}, Bars: {}",
        if bars.iter().any(|b| b.symbol == "ES") {
            "ES"
        } else {
            "NQ"
        },
    ...
    println!(
        "Phase: {} ({} trades/day, {}× size)",
        CHALLENGE.label, CHALLENGE.max_trades_per_day, CHALLENGE.position_size_mult
    );
    println!(
        "Funded: {} ({} trades/day, {}× size)",
        FUNDED.label, FUNDED.max_trades_per_day, FUNDED.position_size_mult
    );
    ...
    for t in &orb_trades {
        ...
        daily_pnl
            .entry(date)
            .or_default()
            .push(if t.r_multiple > 0.0 { pnl } else { -pnl });
    }
    ...
    if !orb_trades.is_empty() {
        ...
        println!(
            "  Trades: {}, WR: {:.1}%, Total R: {:.2}, Avg R: {:.2}",
            orb_trades.len(),
            orb_trades.iter().filter(|t| t.r_multiple > 0.0).count() as f64
                / orb_trades.len() as f64
                * 100.0,
    ...
    if is_daily {
        ...
        if !trend_trades.is_empty() {
            ...
            for t in &trend_trades {
                let date = parse_date(&t.entry_ts);
                let pnl =
                    (t.entry - t.exit).abs() * point_value * CHALLENGE.position_size_mult - 5.0;
                trend_pnl
                    .entry(date)
                    .or_default()
                    .push(if t.r_multiple > 0.0 { pnl } else { -pnl });
            ...
            println!(
                "  Trades: {}, WR: {:.1}%, Total R: {:.2}",
                trend_trades.len(),
                trend_trades.iter().filter(|t| t.r_multiple > 0.0).count() as f64
                    / trend_trades.len() as f64
                    * 100.0,
