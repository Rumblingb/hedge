...
use bill_core::types::{Bar, load_bars_csv};
...
fn main() {
    ...
    let sharpe = if std_r > 0.0 {
        mean_r / std_r * (252.0_f64 * 78.0_f64).sqrt()
    } else {
        0.0
    }; // 78 15m bars per day
    ...
    let sortino = if downside > 0.0 {
        mean_r / downside.sqrt() * (252.0_f64 * 78.0_f64).sqrt()
    } else {
        0.0
    };
    ...
    let long_wr = if !longs.is_empty() {
        longs.iter().filter(|t| t.r_multiple > 0.0).count() as f64 / longs.len() as f64
    } else {
        0.0
    };
    let short_wr = if !shorts.is_empty() {
        shorts.iter().filter(|t| t.r_multiple > 0.0).count() as f64 / shorts.len() as f64
    } else {
        0.0
    };
    let long_avg_r = if !longs.is_empty() {
        longs.iter().map(|t| t.r_multiple).sum::<f64>() / longs.len() as f64
    } else {
        0.0
    };
    let short_avg_r = if !shorts.is_empty() {
        shorts.iter().map(|t| t.r_multiple).sum::<f64>() / shorts.len() as f64
    } else {
        0.0
    };
    ...
    let kelly = if avg_loss > 0.0 {
        wr - (1.0 - wr) / (avg_win / avg_loss).max(0.01)
    } else {
        0.0
    };
    ...
    println!(
        "  Median R: {:.4} | P10: {:.4} | P90: {:.4}",
        median_r, p10_r, p90_r
    );
    ...
    println!(
        "  WR stable > 50% (95% confidence): {}",
        if wr_stable { "✅ YES" } else { "⚠️ NO" }
    );

    // Profit distribution
    let positive_r = r_values
        .iter()
        .filter(|&&r| r > 0.0)
        .copied()
        .collect::<Vec<_>>();
    let negative_r = r_values
        .iter()
        .filter(|&&r| r <= 0.0)
        .copied()
        .collect::<Vec<_>>();
    ...
    println!("  % of profit from top 10% of trades: {:.1}%", {
        ...
        let top10: f64 = sorted
            .iter()
            .take((sorted.len() as f64 * 0.1) as usize)
            .sum();
        if total_pos_r > 0.0 {
            top10 / total_pos_r * 100.0
        } else {
            0.0
        }
    });
    ...
    for comm in &[0.0, 2.5, 5.0, 10.0] {
        ...
        let net_r: f64 = trades
            .iter()
            .map(|t| {
                let pts = (t.entry - t.exit).abs();
                let pnl = if t.r_multiple > 0.0 {
                    pts * gross_per_bar - comm
                } else {
                    -(pts * gross_per_bar + comm)
                };
    ...
    if wr_stable && sharpe > 1.0 && profit_factor > 1.2 && expectancy > 0.0 {
        ...
        if !wr_stable {
            concerns.push("WR not stable at 95% confidence");
        }
        if sharpe <= 1.0 {
            concerns.push("Sharpe ≤ 1.0");
        }
        if profit_factor <= 1.2 {
            concerns.push("Profit Factor ≤ 1.2");
        }
        if expectancy <= 0.0 {
            concerns.push("Expectancy ≤ 0.0");
        }
