...
- All `run_bill_cli` wrapper invocations apply bounded Mac process limits and a watchdog. Tune `BILL_PROCESS_TIMEOUT_SECONDS`, `BILL_PROCESS_KILL_GRACE_SECONDS`, `BILL_PROCESS_CPU_LIMIT_SECONDS`, `BILL_PROCESS_MAX_OPEN_FILES`, `BILL_PROCESS_MAX_USER_PROCESSES`, and `BILL_PROCESS_NICE` in the secure env file.
...
- `bill-trading-day-premarket-scheduled` is the n8n-friendly weekday premarket lane. It pulls SearXNG news, free macro proxies, red-folder events, and an optional OpenRouter free-model advisory into `.rumbling-hedge/research/premarket/premarket-brief.latest.json`, then refreshes autonomy status and the OpenJarvis board. The LLM summary is advisory-only and cannot authorize trades.
...
- `bill-strategy-lab-scheduled` also refreshes `alpha-lab` by default so the point-in-time feature store and train/test alpha candidates are continuously updated; set `BILL_STRATEGY_LAB_RUN_ALPHA_LAB=false` to disable that light pass.
...
- `npm run bill:pm-futures-bridge` writes `.rumbling-hedge/state/pm-futures-bridge.latest.json`, joining prediction-market/copy-demo context into futures symbols as indicator-only regime input. It never grants execution authority.
- `npm run bill:alpha-lab -- <normalized-futures-csv>` writes point-in-time feature rows to `.rumbling-hedge/features/futures-alpha-features.latest.jsonl` and ranked train/test alpha candidates to `.rumbling-hedge/state/alpha-lab.latest.json`, including feature version, purged walk-forward folds, regime checks, and paper-only strategy definitions for survivors.
- `npm run bill:native-core-build` builds the C++20 `bill-core` gate binary under `.rumbling-hedge/native/`; it is a deterministic risk/gate evaluator and still defers live permission to kill-switch plus live-readiness gates.
